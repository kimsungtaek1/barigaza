...
class AdminNoticeTab extends StatefulWidget {
  ...
  const AdminNoticeTab({
    Key? key,
    required this.isSelectionMode,
  }) : super(key: key);

  @override
  _AdminNoticeTabState createState() => _AdminNoticeTabState();
}
...
class _AdminNoticeTabState extends State<AdminNoticeTab> {
  ...
  @override
  void didUpdateWidget(AdminNoticeTab oldWidget) {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('공지사항 로드 중 오류가 발생했습니다'),
    ...
    try {
      final confirmed = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text('선택한 ${_selectedPosts.length}개의 공지사항을 삭제하시겠습니까?'),
          actions: [
            TextButton(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('선택한 공지사항이 삭제되었습니다')),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  void _showPostDetail(DocumentSnapshot post) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => _NoticeDetailScreen(
          post: post,
  ...
  void _navigateToWriteScreen(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => _NoticeWriteScreen(
...
class _NoticeDetailScreen extends StatefulWidget {
...
class _NoticeDetailScreenState extends State<_NoticeDetailScreen> {
  ...
  Future<void> _updatePost() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('공지사항 수정 중 오류가 발생했습니다'),
            backgroundColor: Colors.red,
          ),
        );
      }
...
class _NoticeWriteScreen extends StatefulWidget {
  final VoidCallback onPostCreated;

  const _NoticeWriteScreen({
    Key? key,
...
class _NoticeWriteScreenState extends State<_NoticeWriteScreen> {
  final _formKey = GlobalKey<FormState>();
  ...
  Future<String?> _uploadImage() async {
    ...
    while (attempts < maxRetries) {
      try {
        ...
        final String fileName = 'notice_${DateTime.now().millisecondsSinceEpoch}.$fileExtension';
  ...
  Future<void> _createPost() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('공지사항 등록 중 오류가 발생했습니다: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
